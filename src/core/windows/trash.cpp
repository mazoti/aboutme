...
std::wostream& trash() noexcept{
	...
	IShellFolder *desktop_folder_ptr = nullptr,
	         *recycle_bin_folder_ptr = nullptr;
	LPITEMIDLIST recycle_bin_PID_ptr = nullptr,
	                    item_PID_ptr = nullptr;

	std::vector<std::wstring> trash_data;

	// Initializes COM library
	if(FAILED(CoInitialize(nullptr))) return std::wcout << i18n_system::ERROR_TRASH_COM_INIT << L"\n\n";
	...
	if(FAILED(SHGetDesktopFolder(&desktop_folder_ptr)))
		return std::wcout << i18n_system::ERROR_TRASH_DESKTOP_FOLDER << L"\n\n";

	std::unique_ptr<IShellFolder, releaser<IShellFolder>> df_ptr(desktop_folder_ptr);

	// Gets the Recycle Bin location
	if(FAILED(SHGetSpecialFolderLocation(nullptr, CSIDL_BITBUCKET, &recycle_bin_PID_ptr)))
		return std::wcout << i18n_system::ERROR_TRASH_LOCATION << L"\n\n";
	...
	if(FAILED(desktop_folder_ptr->BindToObject(recycle_bin_PID_ptr, nullptr, IID_IShellFolder, reinterpret_cast<void**>
		(&recycle_bin_folder_ptr)))) return std::wcout << i18n_system::ERROR_TRASH_BIND << L"\n\n";

	std::unique_ptr<IShellFolder, releaser<IShellFolder>> rb_folder_ptr(recycle_bin_folder_ptr);
	...
	if(FAILED(recycle_bin_folder_ptr->EnumObjects(nullptr, SHCONTF_FOLDERS | SHCONTF_NONFOLDERS, &enum_ptr)))
		return std::wcout << i18n_system::ERROR_TRASH_ENUM << L"\n\n";

	std::unique_ptr<IEnumIDList, releaser<IEnumIDList>> enum_list_ptr(enum_ptr);
