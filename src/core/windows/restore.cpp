...
struct couninitializer_restore{
...
std::wostream& restore() noexcept{
	...
	if(FAILED(CoInitializeEx(nullptr, COINIT_MULTITHREADED)))
		return std::wcerr << ERROR_RESTORE_COM_INIT << std::endl << std::endl;
	std::unique_ptr<void, couninitializer_restore> result_handle_ptr(reinterpret_cast<void*>(1));
	...
	if(FAILED(CoCreateInstance(CLSID_WbemLocator, nullptr, CLSCTX_INPROC_SERVER, IID_IWbemLocator,
		...
		return std::wcerr << ERROR_RESTORE_WMI_INIT << std::endl << std::endl;
	...
	if(FAILED(locator_pointer->ConnectServer(_bstr_t(L"ROOT\\DEFAULT"), nullptr, nullptr, nullptr, 0, nullptr, nullptr,
		&svc_pointer))) return std::wcerr << ERROR_RESTORE_WMI_CONNECT << std::endl << std::endl;
	...
	if(FAILED(CoSetProxyBlanket(svc_pointer, RPC_C_AUTHN_WINNT, RPC_C_AUTHZ_NONE, nullptr, RPC_C_AUTHN_LEVEL_CALL,
		...
		return std::wcerr << ERROR_RESTORE_SECURITY_LEVEL << std::endl << std::endl;
	...
	if(FAILED(svc_pointer->ExecQuery(bstr_t("WQL"), bstr_t(L"SELECT CreationTime, Description FROM SystemRestore"),
		...
		return std::wcerr << ERROR_RESTORE_QUERY << std::endl << std::endl;
	...
	while(enumerator_pointer){
		...
		woss.str(L"");
		if(SUCCEEDED(clsobj_pointer->Get(L"CreationTime", 0, &variant_property, nullptr, nullptr))){
		...
		woss.str(L"");
		if(SUCCEEDED(clsobj_pointer->Get(L"Description", 0, &variant_property, nullptr, nullptr))){
			if(variant_property.vt == VT_BSTR){
				woss << variant_property.bstrVal;
				VariantClear(&variant_property);
			}
